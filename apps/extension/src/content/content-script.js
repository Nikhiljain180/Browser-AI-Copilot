...
async function executeTool(toolName, toolInput) {
  ...
  if (!TOOL_REGISTRY[toolName]) {
    throw new Error(
      `Unknown tool: "${toolName}". Available tools: ${Object.keys(TOOL_REGISTRY).join(', ')}`,
    );
  }
...
const TOOL_REGISTRY = {
  ...
  draft_reply: {
    ...
    params: {
      required: [],
      optional: ['selector', 'draft', 'context', 'tone', 'mode', 'format', 'signature'],
    },
  ...
  reset_form: {
    ...
    params: {
      required: [],
      optional: ['selector', 'mode', 'fields', 'clearValidation', 'clearVisualState'],
    },
  ...
  try {
    ...
    return {
      ...
      info: {
        ...
        data:
          element.dataset && Object.keys(element.dataset).length > 0
            ? { ...element.dataset }
            : undefined,
